...
@section('content')
    ...
    <div id="editDetailsModal" class="modal">
        ...
        <div class="modal-content max-w-2xl p-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <form action="{{ route('hr.tickets.update_details', $ticket->ticket_id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="space-y-8">
    ...
    <form id="reopenForm" action="{{ route('hr.tickets.update_status', $ticket->ticket_id) }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="status_id" value="{{ $statusOpen }}">
        <input type="hidden" name="ticket_remarks" value="Ticket reopened by HR.">
    </form>
