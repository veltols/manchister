...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-display font-bold text-slate-800">Attendance Tracking</h2>
            <p class="text-sm text-slate-500 mt-1">{{ $attendances->total() }} total records</p>
        </div>
        <button onclick="openModal('addAttendanceModal')" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">
        ...
        <div class="overflow-x-auto">
            <table class="premium-table w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="font-mono text-sm font-semibold text-slate-600">#{{ $attendance->attendance_id }}</span></td>
                        <td>
                            <div class="flex items-center gap-3">
                        ...
                        <td><span class="text-sm text-slate-600">{{ $attendance->checkin_date ? $attendance->checkin_date->format('M d, Y') : '-' }}</span></td>
                        <td>
                        ...
                        <td><span class="text-sm text-slate-400">{{ $attendance->added_date ? $attendance->added_date->format('Y-m-d') : '-' }}</span></td>
                    </tr>
                    @empty
                    <tr>
...
<div class="modal" id="addAttendanceModal">
    ...
    <div class="modal-content max-w-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-display font-bold text-slate-800">Add Attendance</h2>
            <button onclick="closeModal('addAttendanceModal')" class="w-10 h-10 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="checkin_date" value="{{ date('Y-m-d') }}" class="premium-input w-full px-4 py-3 text-sm" required>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="attendance_remarks" rows="2" class="premium-input w-full px-4 py-3 text-sm" placeholder="Optional remarks"></textarea>
                </div>
            </div>

            <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-slate-200">
                <button type="button" onclick="closeModal('addAttendanceModal')" class="px-6 py-3 rounded-xl text-slate-600 hover:bg-slate-100 font-semibold transition-colors">Cancel</button>
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">Save Entry</button>
            </div>
        </form>
    </div>
</div>
