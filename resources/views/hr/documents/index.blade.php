...
<div class="space-y-6">
    ...
    <!-- Filter -->
    <div>
        <form action="{{ route('hr.documents.index') }}" method="GET">
            <select name="type_id" onchange="this.form.submit()" class="premium-input px-4 py-3 text-sm">
                <option value="">All Types</option>
                @foreach($types as $type)
                    <option value="{{ $type->document_type_id }}" {{ request('type_id') == $type->document_type_id ? 'selected' : '' }}>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="premium-card p-6 relative group">
            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <form action="{{ route('hr.documents.destroy', $doc->document_id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                    @csrf
                    @method('DELETE')
            ...
            <div class="flex items-start gap-4 mb-4">
                <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white shadow-md">
            ...
            <a href="{{ asset('uploads/' . $doc->document_attachment) }}" target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 border-2 border-indigo-200 text-indigo-600 font-semibold rounded-xl hover:bg-indigo-50 transition-colors">
        ...
        <form action="{{ route('hr.documents.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="document_description" rows="2" class="premium-input w-full px-4 py-3 text-sm"></textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Attachment</label>
                    <input type="file" name="document_attachment" class="premium-input w-full px-4 py-3 text-sm" required>
                </div>
            </div>
