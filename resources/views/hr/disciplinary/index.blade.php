...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <button onclick="openModal('addDAModal')"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-slate-800">{{ $da->employee->first_name ?? 'Unknown' }}
                                            {{ $da->employee->last_name ?? '' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r {{ $warningConfig['bg'] }} text-white text-xs font-bold shadow-md">
                                ...
                                <td class="text-center">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-slate-100 text-slate-700 text-xs font-medium">
                                        {{ $da->status->da_status_name ?? 'Unknown' }}
                                    </span>
                                </td>
                                <td><span class="text-sm text-slate-600 truncate max-w-xs block"
                                        title="{{ $da->da_remark }}">{{ $da->da_remark }}</span></td>
                                <td>
    ...
    <div class="modal" id="addDAModal">
        ...
        <div class="modal-content max-w-lg p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <button onclick="closeModal('addDAModal')"
            ...
            <form action="{{ route('hr.disciplinary.store') }}" method="POST">
                ...
                <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-slate-200">
                    <button type="button" onclick="closeModal('addDAModal')"
    ...
    <div class="modal" id="editDAModal">
        ...
        <div class="modal-content max-w-md p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <button onclick="closeModal('editDAModal')"
                    class="w-10 h-10 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
                    <i class="fa-solid fa-times text-xl"></i>
                </button>
            </div>
            ...
            <form id="editDAForm" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Remarks</label>
                        <textarea name="da_remark" id="edit_remark" rows="3"
                            class="premium-input w-full px-4 py-3 text-sm"></textarea>
                    </div>
                </div>

                <button type="submit"
                    class="w-full mt-6 px-6 py-3 premium-button from-indigo-600 to-purple-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200">Update</button>
            </form>
        </div>
    </div>
