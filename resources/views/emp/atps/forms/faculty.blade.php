        ...
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-6">
                    <div
                        class="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 p-0.5 shadow-lg shadow-emerald-100">
                        <div
                            class="w-full h-full bg-white rounded-[14px] flex items-center justify-center overflow-hidden text-emerald-600">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($faculty as $member)
                    @php
                        ...
                        $role = $roleMap[$member->faculty_type_id] ?? ['label' => 'Staff Member', 'icon' => 'fa-user-tie', 'color' => 'slate'];
                    @endphp
                    <div
                        class="premium-card p-6 flex items-start gap-4 hover:shadow-xl hover:shadow-{{ $role['color'] }}-100/20 transition-all">
                        <div
                            class="w-12 h-12 rounded-xl bg-{{ $role['color'] }}-50 flex items-center justify-center text-{{ $role['color'] }}-600 shrink-0">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <span
                                    class="px-2 py-0.5 bg-{{ $role['color'] }}-100 text-{{ $role['color'] }}-700 rounded text-[9px] font-black uppercase tracking-widest">
                                    {{ $role['label'] }}
                                </span>
                            </div>
                ...
                @empty
                    <div class="col-span-full premium-card p-20 text-center">
                        ...
                        <p class="text-slate-400 max-w-md mx-auto">The faculty list for this Training Provider is currently
                            empty or has not been populated yet.</p>
                    </div>
                @endforelse
            </div>
