...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <button onclick="openModal('addTaskModal')" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-brand text-white font-bold rounded-xl shadow-lg shadow-brand/20 hover:shadow-brand/40 hover:scale-105 transition-all duration-200">
    ...
    <div class="premium-card p-4">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="flex gap-2">
                <a href="{{ route('emp.tasks.index', ['view_mode' => 'my_tasks']) }}" class="px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all {{ $viewMode == 'my_tasks' ? 'bg-gradient-brand text-white shadow-lg' : 'text-slate-500 hover:text-brand-dark' }}">
                    Assigned to Me
                </a>
                <a href="{{ route('emp.tasks.index', ['view_mode' => 'others_tasks']) }}" class="px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-all {{ $viewMode == 'others_tasks' ? 'bg-gradient-brand text-white shadow-lg' : 'text-slate-500 hover:text-brand-dark' }}">
                    Assigned by Me
                </a>
            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('emp.tasks.show', $task->task_id) }}" class="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-600 to-purple-600 text-white flex items-center justify-center hover:scale-110 transition-all shadow-md" title="View Details">
                                    <i class="fa-solid fa-eye text-xs"></i>
                                </a>
                                <button @click="openStatusModal({{ $task->task_id }}, {{ $task->status_id }})" class="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 text-white flex items-center justify-center hover:scale-110 transition-all shadow-md" title="Update Status">
                                    <i class="fa-solid fa-arrows-rotate text-xs"></i>
                                </button>
                                <button @click="openProgressModal({{ $task->task_id }}, {{ $task->task_progress }})" class="w-8 h-8 rounded-lg bg-gradient-brand text-white flex items-center justify-center hover:scale-110 transition-all shadow-md" title="Update Progress">
