...
@section('title', 'Form 017')
@section('subtitle', 'Assessors/Trainer Interview Questions')
...
@section('content')
    <div class="max-w-5xl mx-auto space-y-8 animate-fade-in-up">
        ...
        <div
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">Assessors/Trainer Interview
                    </p>
                </div>
            </div>
        ...
        <form action="{{ route('eqa.forms.store', ['form_id' => '017', 'atp_id' => $atp->atp_id]) }}" method="POST"
            class="space-y-8">
            @csrf
            ...
            <div class="premium-card p-6">
                <div class="space-y-2">
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Assessor
                        Name</label>
                    <div class="relative">
                        <i class="fa-solid fa-user absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                        <input type="text" name="assessor_name"
                            class="premium-input w-full pl-11 focus:border-brand focus:ring-brand/5"
                            value="{{ $formData->assessor_name ?? '' }}" placeholder="Enter name of person interviewed...">
                    </div>
                </div>
            </div>
            ...
            <div class="premium-card p-0 overflow-hidden">
                ...
                <div class="p-8 space-y-6">
                    <div id="qa-container" class="space-y-6">
                        @php
                            $questions = isset($formData->questions) ? json_decode($formData->questions, true) : [''];
                            $answers = isset($formData->answers) ? json_decode($formData->answers, true) : [''];
                        @endphp
                        ...
                        @foreach($questions as $index => $question)
                            <div
                                ...
                                <div class="grid grid-cols-1 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        <label
                                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Answer
                                            / Observation</label>
                                        <textarea name="answers[]" rows="3"
                                            class="premium-input w-full resize-none bg-white focus:border-brand focus:ring-brand/5"
                                            placeholder="Record answer here...">{{ $answers[$index] ?? '' }}</textarea>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div
                ...
                <div class="flex items-center gap-3 text-slate-400">
                    <i class="fa-solid fa-microphone-lines text-brand"></i>
                    <span class="text-xs font-bold uppercase tracking-widest">Interview Record</span>
                </div>
                <div class="flex items-center gap-4">
                    <button type="reset"
        ...
        function addQaRow() {
            ...
            row.innerHTML = `
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Answer / Observation</label>
                            <textarea name="answers[]" rows="3" class="premium-input w-full resize-none bg-white focus:border-brand focus:ring-brand/5" placeholder="Record answer here..."></textarea>
                        </div>
                    </div>
                `;
