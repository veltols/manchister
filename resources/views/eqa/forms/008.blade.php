...
@section('content')
    <div class="max-w-6xl mx-auto space-y-8 animate-fade-in-up">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/40 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">External Quality Assurance Visit Planner</p>
                </div>
            </div>
        ...
        <form action="{{ route('eqa.forms.store', ['form_id' => '008', 'atp_id' => $atp->atp_id]) }}" method="POST" class="space-y-8">
            @csrf

            <!-- Section 1: Institutional Context (Read Only) -->
            <div class="premium-card overflow-hidden">
                <div class="bg-slate-50 border-b border-slate-100 px-8 py-4">
                    <h3 class="font-bold text-slate-600 uppercase text-[10px] tracking-widest flex items-center gap-2">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-1">
                        ...
                        <div class="p-4 rounded-xl bg-slate-50 border border-slate-100 text-premium font-bold text-sm shadow-inner group transition-all">
                            <i class="fa-solid fa-building-columns text-brand/30 mr-2 group-hover:text-brand transition-colors"></i>
                            {{ $atp->atp_name }}
                        </div>
                    </div>
                    <div class="space-y-1">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Author of SED</label>
                        <div class="p-4 rounded-xl bg-slate-50 border border-slate-100 text-premium font-bold text-sm shadow-inner group transition-all">
            ...
            <div class="premium-card p-8 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-32 h-32 bg-brand/5 rounded-full -mr-16 -mt-16 blur-3xl"></div>
                <h3 class="font-bold text-premium mb-8 uppercase text-xs tracking-[0.2em] border-l-4 border-l-brand pl-4">Visit Logistics</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="eqa_visit_date" class="premium-input w-full pl-11"
                                value="{{ $formData->eqa_visit_date ?? '' }}" required>
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Visit Type</label>
                        <div class="relative">
                            <i class="fa-solid fa-tag absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                            <select name="visit_type" class="premium-input w-full pl-11">
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Activity Mode</label>
                        <div class="relative">
                            <i class="fa-solid fa-bolt absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                            <select name="activity" class="premium-input w-full pl-11">
                                <option value="">Please Select</option>
                    ...
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Length (Hours)</label>
                        <div class="relative">
                            <i class="fa-solid fa-clock absolute left-4 top-1/2 -translate-y-1/2 text-slate-300"></i>
                            <select name="visit_length" class="premium-input w-full pl-11">
                                <option value="">Please Select</option>
                                @for ($i = 1; $i <= 8; $i++)
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-8">
                    <div class="premium-card p-8">
                        <h3 class="font-bold text-premium mb-6 uppercase text-xs tracking-[0.2em] flex items-center gap-2">
                            <i class="fa-solid fa-clipboard-list text-brand"></i>
                            Scope & Agenda
                        </h3>
                        <div class="space-y-6">
                            <div class="space-y-2">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Scope of Visit</label>
                                <textarea name="visit_scope" rows="5" class="premium-input w-full resize-none leading-relaxed"
                                    placeholder="Define the primary objectives and scope...">{{ $formData->visit_scope ?? '' }}</textarea>
                            </div>
                            <div class="space-y-2">
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Detailed Agenda</label>
                                <textarea name="visit_agenda" rows="8" class="premium-input w-full resize-none leading-relaxed"
                ...
                <div class="space-y-8">
                    <div class="premium-card p-8 bg-slate-50/50">
                        <h3 class="font-bold text-premium mb-6 uppercase text-xs tracking-widest flex items-center gap-2">
                            <i class="fa-solid fa-comment-dots text-brand"></i>
                            Observations
                        </h3>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Additional Notes</label>
                            <textarea name="visit_comment" rows="10" class="premium-input w-full bg-white resize-none"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="premium-card overflow-hidden">
                    <div class="bg-slate-50 border-b border-slate-100 px-8 py-5 flex items-center justify-between">
                        <h3 class="font-bold text-slate-600 uppercase text-[10px] tracking-widest">Qualifications Offered</h3>
                        <span class="px-2 py-0.5 rounded bg-brand/10 text-brand text-[10px] font-bold">{{ count($qualifications) }}</span>
                    </div>
                    <div class="p-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                        @forelse($qualifications as $q)
                            <div class="mb-4 last:mb-0 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all group">
                                <div class="flex items-start gap-4">
                                    <div class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-brand group-hover:text-white transition-all shadow-inner">
                ...
                <div class="premium-card overflow-hidden">
                    <div class="bg-slate-50 border-b border-slate-100 px-8 py-5 flex items-center justify-between">
                        ...
                        <span class="px-2 py-0.5 rounded bg-amber-100 text-amber-600 text-[10px] font-bold">{{ count($faculties) }}</span>
                    </div>
                    <div class="p-6 max-h-[400px] overflow-y-auto custom-scrollbar">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[10px] text-slate-400 mt-0.5 font-bold uppercase">{{ $f->faculty_spec }}</div>
                                        </div>
                                    </div>
                                    <span class="px-3 py-1 rounded-lg bg-slate-50 border border-slate-100 text-[10px] font-black text-slate-500 uppercase tracking-tighter">
