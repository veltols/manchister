...
<aside class="w-64 flex-shrink-0 flex flex-col shadow-2xl relative z-20 hidden md:flex sidebar-gradient-bg">
    ...
    <div class="h-44 flex items-center justify-center px-4 text-center relative overflow-hidden">
        ...
        <a href="{{ route('dashboard') }}"
            class="relative z-10 transition-all duration-500 hover:scale-105 active:scale-95">
            @php
            ...
            @endphp
            <img src="{{ $sLogoUrl }}" alt="IQC Logo"
                class="h-28 w-auto object-contain sidebar-logo filter drop-shadow-[0_10px_20px_rgba(0,0,0,0.08)]">
        </a>
    </div>

    <!-- Navigation -->
    <nav class="flex-1 flex flex-col py-4 px-3 overflow-y-auto relative z-10">
        @if($user && $user->user_type === 'atp')
            @include('layouts.sidebar.menus.atp')
        @elseif($user && in_array($user->user_type, ['emp', 'eqa']))
