...
@section('content')
    <div class="space-y-6">
        ...
        <div class="premium-card p-2 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <form action="{{ route('admin.tickets.index') }}" method="GET" class="flex gap-2 w-full sm:w-auto">
                @if(request('stt'))
                ...
                <div id="resolved-slider" class="flex items-center gap-3 overflow-x-auto pb-3 no-scrollbar scroll-smooth">
                    ...
                    @foreach($resolvedMonths as $month)
                        <a href="{{ route('admin.tickets.index', ['stt' => 3, 'month' => $month->month_value]) }}"
                            ...
                            <span>{{ \Carbon\Carbon::parse($month->month_value)->format('F Y') }}</span>
                            
                            <!-- Count Badge -->
                            @if($month->total > 0)
                                <span class="inline-flex items-center justify-center min-w-[20px] h-[20px] px-1.5 rounded-full text-[10px] font-bold shadow-sm transition-all {{ request('month') == $month->month_value ? 'bg-white text-indigo-600' : 'bg-indigo-600 text-white' }}">
                                    {{ $month->total }}
                                </span>
                            @else
                                <span class="inline-flex items-center justify-center min-w-[20px] h-[20px] px-1.5 rounded-full text-[10px] font-bold border {{ request('month') == $month->month_value ? 'bg-white/20 text-white border-white/30' : 'bg-slate-50 text-slate-400 border-slate-100' }}">
                                    0
                                </span>
                            @endif
                        </a>
