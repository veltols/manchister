    ...
    <div class="premium-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="premium-table w-full">
                <thead><tr>
                    <th>Ref</th><th>Employee</th><th>Probation</th><th>Manager</th><th>Rating</th><th class="text-center">Status</th><th class="text-center">Action</th>
                </tr></thead>
                <tbody>
                @forelse($reviews as $review)
                @php $colors=['pending_manager'=>'from-amber-400 to-amber-600','reviewed'=>'from-blue-500 to-indigo-600','approved'=>'from-emerald-500 to-green-600','rejected'=>'from-rose-500 to-red-600']; $color=$colors[$review->status]??'from-slate-400 to-slate-600'; @endphp
                <tr>
                    <td><span class="font-mono text-xs text-slate-500">#{{ $review->review_id }}</span></td>
                    <td><span class="font-semibold text-slate-700 text-sm">{{ $review->employee->first_name ?? 'N/A' }} {{ $review->employee->last_name ?? '' }}</span></td>
                    ...
                    <td class="text-sm text-slate-600">{{ $review->lineManager->first_name ?? 'N/A' }} {{ $review->lineManager->last_name ?? '' }}</td>
                ...
                <tr><td colspan="7" class="text-center py-16">
                    <div class="flex flex-col items-center gap-3"><div class="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center"><i class="fa-solid fa-gavel text-2xl text-slate-300"></i></div><p class="text-slate-500">No probation reviews assigned to you</p></div>
                </td></tr>
                @endforelse
                </tbody>
            </table>
