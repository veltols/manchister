...
$primaryKey = "local_map_id";
$tableName = "m_strategic_plans_internal_maps";
$currentPage = 1;
$recordsPerPage = 10;
$COND = "";
$plan_id = 0;
...
if ($IS_LOGGED == true && $USER_ID != 0 && $plan_id != 0) {
	...
	$qu_table_related_sel = "SELECT `$tableName`.*, 
							...
							INNER JOIN `m_strategic_plans_milestones` ON `$tableName`.`milestone_id` = `m_strategic_plans_milestones`.`milestone_id` 
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"milestone_title" => $ARRAY_SRC['milestone_title'],
					"task_title" => $ARRAY_SRC['task_title'],
					"task_description" => $ARRAY_SRC['task_description'],
					"start_date" => $stArr[0],
					"end_date" => $enArr[0],
