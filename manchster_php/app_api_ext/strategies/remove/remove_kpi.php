...
if ($item_id != 0) {
	...
	//check if mapped
	$qu_m_strategic_plans_internal_maps_sel = "SELECT * FROM  `m_strategic_plans_internal_maps` WHERE `kpi_id` = $item_id";
	...
	if( mysqli_num_rows($qu_m_strategic_plans_internal_maps_EXE) > 0 ){
		...
		$IAM_ARRAY['message'] = "KPI is mapped, please delete the map first";
		
	} else {
		//remove all milestones
		$qu_m_strategic_plans_milestones_del = "DELETE FROM `m_strategic_plans_milestones` WHERE `kpi_id` = $item_id";
		if (mysqli_query($KONN, $qu_m_strategic_plans_milestones_del)) {


			//remove all kpis
			$qu_m_strategic_plans_kpis_del = "DELETE FROM `m_strategic_plans_kpis` WHERE `kpi_id` = $item_id";
			if (mysqli_query($KONN, $qu_m_strategic_plans_kpis_del)) {
				...
				$IAM_ARRAY['success'] = false;
				$IAM_ARRAY['message'] = "ERR-TACT-23232";
			}
