...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if( $extra_id == 0 ){
		...
		$COND = $COND."( (`$tableName`.`atp_status_id` = 1) OR (`$tableName`.`atp_status_id` = 2) ) AND ";
	} else if( $extra_id == 3 ){
		$COND = $COND."(`$tableName`.`atp_status_id` = 3) AND ";
	} else if( $extra_id == 4 ){
		$COND = $COND."(`$tableName`.`atp_status_id` = 4) AND ";
	} else if( $extra_id == 5 ){
		$COND = $COND."(`$tableName`.`atp_status_id` = 5) AND ";
	}
	...
	$qu_table_related_sel = "SELECT `$tableName`.*, 
									...
									`atps_list_status`.`atp_status_name` AS `atp_status`,
									`atps_list_status`.`atp_status_description` AS `atp_status_desc`

							FROM  `$tableName` 
							INNER JOIN `employees_list` ON `$tableName`.`added_by` = `employees_list`.`employee_id` 
							INNER JOIN `atps_list_status` ON `$tableName`.`atp_status_id` = `atps_list_status`.`atp_status_id` 
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"status_id" => (int) $ARRAY_SRC['atp_status_id'],
