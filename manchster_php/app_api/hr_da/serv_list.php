...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	$da_id = 0;
	if (isset($_REQUEST['da_id'])) {
		$da_id = (int) test_inputs($_REQUEST['da_id']);
		if ($da_id != 0) {
			$COND = $COND . "(`$tableName`.`da_id` = $da_id) AND ";
		}
	}
	...
	$qu_table_related_sel = "SELECT `$tableName`.*, 
									...
									`hr_disp_actions_types`.`da_type_text` AS `da_type`, 
									`hr_disp_actions_status`.`da_status_name` AS `da_status`, 
									`hr_disp_actions_warnings`.`da_warning_name` AS `da_warning`

							FROM  `$tableName` 
							INNER JOIN `employees_list` `a` ON `$tableName`.`employee_id` = `a`.`employee_id` 
							INNER JOIN `hr_disp_actions_types` ON `$tableName`.`da_type_id` = `hr_disp_actions_types`.`da_type_id` 
							INNER JOIN `hr_disp_actions_status` ON `$tableName`.`da_status_id` = `hr_disp_actions_status`.`da_status_id` 
							INNER JOIN `hr_disp_actions_warnings` ON `$tableName`.`da_warning_id` = `hr_disp_actions_warnings`.`da_warning_id` 
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"da_type" => decryptData($ARRAY_SRC['da_type']),
					"da_warning" => $ARRAY_SRC['da_warning'],
					"da_warning_id" => $ARRAY_SRC['da_warning_id'],
					"added_date" => $ARRAY_SRC['added_date'],
					"da_type_id" => $ARRAY_SRC['da_type_id'],
					"da_remark" => $ARRAY_SRC['da_remark'],
					"employee_id" => $ARRAY_SRC['employee_id'],
					"da_status" => $ARRAY_SRC['da_status'],
					"da_status_id" => $ARRAY_SRC['da_status_id']
				)
			);
		}
