...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (isset($_POST['platform_name'])) {
		...
		$platform_name = encryptData("" . test_inputs($_POST['platform_name']));
		$platform_purpose = isset($_POST['platform_purpose']) ? encryptData("" . test_inputs($_POST['platform_purpose'])) : '';



		$atpsListContactsQryIns = "INSERT INTO `atps_electronic_systems` (
									`platform_name`, 
									`platform_purpose`, 
									`atp_id` 
								) VALUES ( ?, ?, ? );";
		...
		if ($atpsListContactsStmtIns = mysqli_prepare($KONN, $atpsListContactsQryIns)) {
			if (mysqli_stmt_bind_param($atpsListContactsStmtIns, "ssi", $platform_name, $platform_purpose, $atp_id)) {
				if (mysqli_stmt_execute($atpsListContactsStmtIns)) {

					if (InsertAtpLog("ATP added new Platform", $atp_id, 'atps_list', $USER_ID, 'R&C')) {
