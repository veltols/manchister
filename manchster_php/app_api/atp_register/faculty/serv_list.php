...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	$qu_table_related_sel = "SELECT `$tableName`.*, 
									`atps_list_faculties_types`.`faculty_type_name` AS `faculty_type_name`
									
							FROM  `$tableName` 
							INNER JOIN `atps_list_faculties_types` ON `$tableName`.`faculty_type_id` = `atps_list_faculties_types`.`faculty_type_id` 
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"faculty_name" => decryptData("" . $ARRAY_SRC['faculty_name']),
					"faculty_type_id" => decryptData("" . $ARRAY_SRC['faculty_type_id']),
					"faculty_type_name" => decryptData("" . $ARRAY_SRC['faculty_type_name']),
					"educational_qualifications" => decryptData("" . $ARRAY_SRC['educational_qualifications']),
					"years_experience" => decryptData("" . $ARRAY_SRC['years_experience']),
					"certificate_name" => decryptData("" . $ARRAY_SRC['certificate_name']), 
					"faculty_cv" => decryptData("" . $ARRAY_SRC['faculty_cv']), 
					"faculty_certificate" => decryptData("" . $ARRAY_SRC['faculty_certificate'])
				)
			);
		}
