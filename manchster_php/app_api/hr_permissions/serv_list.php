...
$tableName = "hr_employees_permissions";
$currentPage = 1;
$recordsPerPage = 10;
$COND = "";
$extra_id = 0;
...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (isset($_REQUEST['permission_id'])) {
		$permission_id = (int) test_inputs($_REQUEST['permission_id']);
		if ($permission_id != 0) {
			$COND = $COND . "(`$tableName`.`permission_id` = $permission_id) AND ";
		}
	}
	...
	$qu_table_related_sel = "SELECT `$tableName`.*, 
									...
									`hr_employees_permissions_status`.`permission_status_name` AS `permission_status`

							FROM  `$tableName` 
							INNER JOIN `employees_list` `a` ON `$tableName`.`employee_id` = `a`.`employee_id` 
							INNER JOIN `hr_employees_permissions_status` ON `$tableName`.`permission_status_id` = `hr_employees_permissions_status`.`permission_status_id` 
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"start_time" => $ARRAY_SRC['start_time'],
					"end_time" => $ARRAY_SRC['end_time'],
					"total_hours" => $ARRAY_SRC['total_hours'],
					"permission_remarks" => $ARRAY_SRC['permission_remarks'],
					"employee_id" => $ARRAY_SRC['employee_id'],
					"permission_status" => $ARRAY_SRC['permission_status'],
					"permission_status_id" => $ARRAY_SRC['permission_status_id']
				)
			);
		}
