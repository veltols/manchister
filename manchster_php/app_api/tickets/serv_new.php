<?php

$IAM_ARRAY;

$RES = false;
$idder = "";
$token_value = $falseToken;
if ($IS_LOGGED == true && $USER_ID != 0) {

	if (
		isset($_POST['added_by']) &&
		isset($_POST['ticket_subject']) &&
		isset($_POST['ticket_description']) &&
		isset($_POST['category_id']) &&
		isset($_POST['priority_id'])
	) {


		//todo
		//authorize inputs

		$ticket_id = 0;
		$ticket_subject = "" . test_inputs($_POST['ticket_subject']);
		$ticket_description = "" . test_inputs($_POST['ticket_description']);
		$status_id = 1;
		$added_by = (int) test_inputs($_POST['added_by']);
		$priority_id = (int) test_inputs($_POST['priority_id']);
		$category_id = (int) test_inputs($_POST['category_id']);
		$ticket_added_date = date('Y-m-d H:i:00');


		//$added_by = $USER_ID;


		$department_id = 0;
		$qu_employees_list_sel = "SELECT `department_id` FROM  `employees_list` WHERE `employee_id` = $USER_ID";
		$qu_employees_list_EXE = mysqli_query($KONN, $qu_employees_list_sel);
		if (mysqli_num_rows($qu_employees_list_EXE)) {
			$employees_list_DATA = mysqli_fetch_assoc($qu_employees_list_EXE);
			$department_id = (int) $employees_list_DATA['department_id'];
		}

		//generate REF
		$ticket_ref = "T-" . date("ym") . '';
		$qu_support_tickets_list_sel = "SELECT COUNT(`ticket_id`) AS `total_tickets` FROM  `support_tickets_list`";
		$qu_support_tickets_list_EXE = mysqli_query($KONN, $qu_support_tickets_list_sel);
		if (mysqli_num_rows($qu_support_tickets_list_EXE)) {
			$support_tickets_list_DATA = mysqli_fetch_array($qu_support_tickets_list_EXE);
			$total_tickets = (int) $support_tickets_list_DATA[0];
			$total_tickets++;
			$ticket_ref = $ticket_ref . $total_tickets;
		}

		//check if file is attached
		$ticket_attachment = "no-img.png";
		$allowFileExt = array('pdf', 'jpg', 'png', 'jpeg', 'csv', 'doc', 'docx', 'xls', 'xlsx');

		if (isset($_FILES['ticket_attachment']) && $_FILES['ticket_attachment']["tmp_name"]) {
			$thsName = $_FILES['ticket_attachment']["name"];
			$thsTemp = $_FILES['ticket_attachment']["tmp_name"];
			if (!empty($thsName)) {
				$fileExt = pathinfo("../uploads/" . basename($thsName), PATHINFO_EXTENSION);

				if (in_array($fileExt, $allowFileExt)) {

					//upload file to server
					$upload_res = upload_file('ticket_attachment', 8000, 'uploads', '../');
					if ($upload_res == true) {
						$ticket_attachment = $upload_res;
					} else {
						$IAM_ARRAY['success'] = false;
						$IAM_ARRAY['message'] = "Error in file - 574574568";
					}

				} else {
					$IAM_ARRAY['success'] = false;
					$IAM_ARRAY['message'] = "Error in file format";
				}

			}

		}



		$atpsListQryIns = "INSERT INTO `support_tickets_list` (
											`ticket_ref`, 
											`ticket_subject`, 
											`ticket_description`, 
											`ticket_attachment`, 
											`status_id`, 
											`priority_id`, 
											`category_id`, 
											`ticket_added_date`, 
											`department_id`, 
											`added_by`
											) VALUES (  ?, ?, ?, ?, ?, ?, ?, ?, ?, ? );";

		if ($atpsListStmtIns = mysqli_prepare($KONN, $atpsListQryIns)) {
			if (mysqli_stmt_bind_param($atpsListStmtIns, "ssssiiisii", $ticket_ref, $ticket_subject, $ticket_description, $ticket_attachment, $status_id, $priority_id, $category_id, $ticket_added_date, $department_id, $added_by)) {
				if (mysqli_stmt_execute($atpsListStmtIns)) {
					$ticket_id = (int) mysqli_insert_id($KONN);
					mysqli_stmt_close(statement: $atpsListStmtIns);



					//insert ticket log
					$log_id = 0;
					$log_action = "ticket_Added";
					$log_remark = "---";
					$log_date = date('Y-m-d H:i:00');
					$logger_type = "employees_list";
					$log_dept = "IT";
					$logged_by = $USER_ID;


					if (insertSysLog('support_tickets_list', $ticket_id, $log_action, '---', $logger_type, $logged_by, 'int')) {
						$IAM_ARRAY['success'] = true;
						$IAM_ARRAY['message'] = "Succeed";
					} else {
						reportError('Ticket log failed - 4584864 ', 'employees_list', $EMPLOYEE_ID);
					}

					//notify user
					notifyUser("A new ticket has been added, REF: " . $ticket_ref, 'tickets/list/', $added_by);

					//notify IT
					notifyUser("A new ticket has been added, REF: " . $ticket_ref, 'tickets/list/', 1);













				} else {
					//Execute failed 
					reportError(mysqli_stmt_error($atpsListStmtIns), 'employees_list', $EMPLOYEE_ID);
					$IAM_ARRAY['success'] = false;
					$IAM_ARRAY['message'] = "ERR-12-57";
				}
			} else {
				//bind failed 
				reportError(mysqli_stmt_error($atpsListStmtIns), 'employees_list', $EMPLOYEE_ID);
				$IAM_ARRAY['success'] = false;
				$IAM_ARRAY['message'] = "ERR-12-56";
			}
		} else {
			//prepare failed 
			reportError('support_tickets_list failed to prepare stmt ', 'employees_list', $EMPLOYEE_ID);
			$IAM_ARRAY['success'] = false;
			$IAM_ARRAY['message'] = "ERR-12-55";
		}



	} else {
		//No request
		$IAM_ARRAY['success'] = false;
		$IAM_ARRAY['message'] = "ERR-12-4556";
	}









} else {
	$IAM_ARRAY['success'] = false;
	$IAM_ARRAY['message'] = "ERR-100";
}





header('Content-Type: application/json');
echo json_encode(array($IAM_ARRAY));


