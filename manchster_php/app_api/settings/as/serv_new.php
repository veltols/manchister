...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (
		...
		$atpsListQryIns = "INSERT INTO `z_assets_list_status` (
											`status_name` 
											) VALUES ( ? );";

		if ($atpsListStmtIns = mysqli_prepare($KONN, $atpsListQryIns)) {
			if (mysqli_stmt_bind_param($atpsListStmtIns, "s", $status_name)) {
				if (mysqli_stmt_execute($atpsListStmtIns)) {
					$status_id = (int) mysqli_insert_id($KONN);
			...
			//prepare failed 
			reportError('z_assets_list_status failed to prepare stmt ', 'employees_list', $EMPLOYEE_ID);
			$IAM_ARRAY['success'] = false;
