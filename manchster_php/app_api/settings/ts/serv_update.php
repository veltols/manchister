...
if ($IS_LOGGED == true && $USER_ID != 0 && isset($_POST['status_id']) && isset($_POST['status_name'])) {
	...
	if ($qqq != "") {
		...
		$qu_support_tickets_list_status_updt = "UPDATE `support_tickets_list_status` SET " . $qqq . "  WHERE `status_id` = $status_id;";


		if (mysqli_query($KONN, $qu_support_tickets_list_status_updt)) {
