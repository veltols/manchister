...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (isset($_REQUEST['performance_id'])) {
		$performance_id = (int) test_inputs($_REQUEST['performance_id']);
		if ($performance_id != 0) {
			$COND = $COND . "(`$tableName`.`performance_id` = $performance_id) AND ";
		}
	}
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"performance_object" => $ARRAY_SRC['performance_object'],
					"performance_kpi" => $ARRAY_SRC['performance_kpi'],
					"performance_remark" => $ARRAY_SRC['performance_remark'],
