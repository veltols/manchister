...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (isset($_REQUEST['attendance_id'])) {
		$attendance_id = (int) test_inputs($_REQUEST['attendance_id']);
		if ($attendance_id != 0) {
			$COND = $COND . "(`$tableName`.`attendance_id` = $attendance_id) AND ";
		}
	}
		...
		while ($ARRAY_SRC = mysqli_fetch_assoc($qu_table_related_EXE)) {
			...
			array_push(
				...
				array(
					...
					"checkin_date" => $ARRAY_SRC['checkin_date'],
					"checkin_time" => $ARRAY_SRC['checkin_time'],
					"attendance_remarks" => $ARRAY_SRC['attendance_remarks'],
