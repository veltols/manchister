...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (
		...
		$qu_z_messages_list_files_sel = "SELECT * FROM  `z_messages_list_files` WHERE ((`file_name` = '$file_name') AND (`chat_id` = $chat_id))";
		$qu_z_messages_list_files_EXE = mysqli_query($KONN, $qu_z_messages_list_files_sel);
		$z_messages_list_files_DATA;
		if (mysqli_num_rows($qu_z_messages_list_files_EXE) > 0) {
			//error
					...
					if (in_array($fileExt, $allowFileExt)) {
						...
						if ($upload_res == true) {
							...
							if ($atpsListStmtIns = mysqli_prepare($KONN, $atpsListQryIns)) {
								if (mysqli_stmt_bind_param($atpsListStmtIns, "ssisi", $file_name, $file_path, $added_by, $added_date, $chat_id)) {
