...
if ($IS_LOGGED == true && $USER_ID != 0) {
	...
	if (
		...
		if ($file_id != 0) {
			...
			$file_version = 2;
			$qu_z_messages_list_files_sel = "SELECT COUNT(`file_id`) FROM  `z_messages_list_files` WHERE ((`chat_id` = $chat_id) AND (`main_file_id` = $file_id))";
			...
			if (mysqli_num_rows($qu_z_messages_list_files_EXE)) {
				$z_messages_list_files_DATA = mysqli_fetch_array($qu_z_messages_list_files_EXE);
				$curVersionCount = (int) $z_messages_list_files_DATA[0];
				$file_version = $curVersionCount + 2;
			}
			//get file name
			$file_name = '';
			$qu_name_sel = "SELECT `file_name` FROM  `z_messages_list_files` WHERE ((`chat_id` = $chat_id) AND (`file_id` = $file_id))";
				...
				if (!empty($thsName)) {
					...
					if (in_array($fileExt, $allowFileExt)) {
						...
						if ($upload_res == true) {
							...
							$atpsListQryIns = "INSERT INTO `z_messages_list_files` (
										`main_file_id`, 
										`file_name`, 
										`file_version`, 
										`file_path`, 
							...
							if ($atpsListStmtIns = mysqli_prepare($KONN, $atpsListQryIns)) {
								if (mysqli_stmt_bind_param($atpsListStmtIns, "isisisi", $file_id, $ff, $file_version, $file_path, $added_by, $added_date, $chat_id)) {
									if (mysqli_stmt_execute($atpsListStmtIns)) {
										...
										$qu_z_messages_list_posts_ins = "INSERT INTO `z_messages_list_posts` (
											`post_text`, 
											`post_type`, 
											`post_attachment_id`, 
											`chat_id`, 
										...
										if (!mysqli_query($KONN, $qu_z_messages_list_posts_ins)) {
								...
								//prepare failed 
								reportError('z_messages_list_files failed to prepare stmt ', 'employees_list', $EMPLOYEE_ID);
								$IAM_ARRAY['success'] = false;
