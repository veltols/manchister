<script>
	...
	function loadChatMessages(chat_id, senderName, senderInitials) {
		activeChat = chat_id;
		sName = senderName;
		...
		$('.chat-id-service').val(activeChat);
...
<script>
	...
	function bindChatPosts(data) {
		...
		for (i = 0; i < data.length; i++) {
			...
			var msgTxt = '' + data[i]["post_text"].trim();
			var dt = '';
			var myClass = '';
			...
			var msgTxtDiv = '';
			if( msgTxt != '' ){
				msgTxtDiv = '		<div class="messageContentText">' + data[i]["post_text"] + '</div>';
			}
			if (post_type == 'text') {
