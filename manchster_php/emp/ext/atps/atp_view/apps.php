...
/*
		<div class="tr levelRow" id="levelRow-sss">
						...
						<div class="td">SSS</div>
						<div class="td">SSS</div>
						<div class="td"><a href="<?=$POINTER; ?>atps/view/init/?atp_id=<?=$atp_id; ?>" class="tableActionBtn"><?=lang("Details"); ?></a></div>
		</div>
		*/
		?>
		</div>
...
<script>
function bindthsAppsData1( data ){
	...
	for( i = 0 ; i < data.length ; i++ ){
		var btns = '';
            ...
            //go to app page
            if( is_show_view == 1 ){
                btns = '<a href="<?=$POINTER; ?>ext/atps/view/init/?atp_id=<?=$atp_id; ?>" class="tableActionBtn"><?=lang("Details"); ?></a>';
            }
			...
			var linkBtn = '	<div class="td"><a href="<?=$POINTER; ?>ext/atps/view/' + linker + '/?atp_id=<?=$atp_id; ?>" class="tableActionBtn"><?=lang("Action"); ?></a></div>';
			if( linker == '' ){
				linkBtn = '';
			}
			...
			$('#appsListDt').prepend( dt );
			listCount++;
	}
	
	if( listCount == 0 ){
		$('#appsListDt').html( '<div class="noData"><img src="<?=$POINTER; ?>../uploads/no_data.png" alt="noData" /><br><?=lang("No_records_found"); ?></div>' );
	}
}
		...
		$.ajax({
			...
			success: function(responser){
				...
				if( responser[0].success == true && responser[0].data){
					bindthsAppsData1(responser[0].data);
				} else {
