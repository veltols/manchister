<script>
	...
	function initForms() {
		...
		$('.inputer').each(function () {
			...
			//check if hidden
			var thsType = $(this).attr('data-type');
			if (thsType == 'hidden') {
	...
	var submittedForm = '';
	function submitForm(formId, pageController) {
		var formdata = new FormData();
		var errorCount = 0;
		var notifiedUser = 0;
		submittedForm = formId;
		$('#' + formId + ' .inputer').each(function () {
	...
	function clearForm(formId) {
		...
		$('#' + formId + ' .inputer').each(function () {
			...
			var thsType = $(this).attr('data-type');
			if( thsType == 'text' || thsType == 'text_mixed' || thsType == 'date' ){
				$(this).val('');
			} else if( thsType == 'int' || thsType == 'float' || thsType == 'number' ){
				$(this).val(0);
			} else if( thsType == 'hidden' ){
				//NA
			}
	...
	function completeHandlerA(event) {
		...
		try {
			...
			end_loader('article');
			makeSiteAlert('err', "Failed, please check error log");
			reportError(event.target.responseText);
		}
	...
	function reportError(errText){
		
		
		$.ajax({
			type: 'POST', 
			url: '<?=$POINTER.'report_sys_error'; ?>', 
			dataType : "JSON", 
			data: {'error_text': errText}, 
			success: function(responser){
				
				if( responser[0].success == true && responser[0].data){
					console.log("Error Reporting Succeed");
				} else {
