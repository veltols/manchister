...
$pageDataController = $POINTER . "get_pp_list";
$addNewController = $POINTER . 'add_pp_list';
$updateController = $POINTER . 'update_pp_list';
...
<script>
	async function bindData(data) {
		...
		for (i = 0; i < data.length; i++) {
			var btns = '';
			btns += '<a onclick="getData(' + data[i]["theme_id"] + ');" class="dataActionBtn hasTooltip">' +
				'	<i class="fa-regular fa-eye"></i>' +
				'	<div class="tooltip"><?= lang("Edit"); ?></div>' +
				'</a>';

			//btns= '---';
			var dt = '<div class="tr levelRow" id="dataRow-' + data[i]["theme_id"] + '">' +
				'	<div class="td">' + data[i]["theme_id"] + '</div>' +
				'	<div class="td priority_name">' + data[i]["priority_name"] + '</div>' +
				'	<div class="td">' +
				btns +
				'	</div>' +
				'</div>';
	...
	function getData(cId) {
		var priority_name = $('#dataRow-' + cId + ' .priority_name').text();
		$('.theme_id_at_edit').val(cId);
		$('.priority_name_at_edit').val(priority_name);
		showModal('viewModal');
	}
</script>
		...
		<div class="row pageForm" id="editForm">
			...
			<div class="col-1">
				<div class="formElement">
					<label><?= lang("priority_name", "AAR"); ?></label>
					<input type="hidden" class="inputer theme_id_at_edit" data-name="theme_id" data-den=""
						data-req="1" data-type="hidden">
					<input type="text" class="inputer priority_name_at_edit" data-name="priority_name" data-den=""
	...
	<div class="modalBody">
		<div class="row pageForm" id="AddNewForm">
			...
			<div class="col-1">
				<div class="formElement">
					<label><?= lang("priority_name", "AAR"); ?></label>
					<input type="text" class="inputer" data-name="priority_name" data-den="" data-req="1"
						data-type="text">
				</div>
			</div>
			<!-- ELEMENT END -->
