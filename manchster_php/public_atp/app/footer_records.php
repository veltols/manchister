	...
	<div class="panelFooter">
		...
		<div class="pagintationPages">
			...
			<div class="pageCounter">
				<span class="currentPage" id="currentPage">1</span> <span>-</span> <span id="totalPages" class="totalPages" >11</span>
			</div>
...
function callPageData(){
	...
	if( activeRequest == false ){
		...
		$.ajax({
			...
			dataType : "JSON", 
			method   : "POST", 
			data     : { "currentPage" : currentPage, "recordsPerPage" : recordsPerPage, "extra_id" : <?php if( isset( $extra_id ) ){ echo $extra_id; } else { echo 0; } ?>  }, 
...
function searchRecords(){
		...
		if( activeRequest == false ){
			...
			url      : '<?=$pageDataController; ?>', 
				dataType : "JSON", 
				method   : "POST", 
				data     : { "query_srch" : dt,  "currentPage" : currentPage, "recordsPerPage" : recordsPerPage, "extra_id" : <?php if( isset( $extra_id ) ){ echo $extra_id; } else { echo 0; } ?> }, 
				success: function( RES ){
					doStuff( RES );
				}, 
				error: function(){
