...
$editFacultyCVController = $POINTER . 'update_atp_faculty_files';
...
<div class="pageHeader">
	...
	<div class="pageOptions">
		<a class="pageLink" onclick="addNewFaculty();" style="width: 15% !important;text-align: center;">
				...
				<div class="table">
					<div class="tableHeader">
						<div class="tr">
							...
							<div class="th"><?= lang("CV"); ?></div>
							<div class="th"><?= lang("Certificate"); ?></div>
...
<script>
	async function bindData(data) {
		...
		for (i = 0; i < data.length; i++) {
			var btns = '';
			var thsCV = data[i]["faculty_cv"];
			var thsCF = data[i]["faculty_certificate"];
			...
			btns += '<a onclick="editFacultyCV(' + data[i]["faculty_id"] + ');" class="dataActionBtn hasTooltip">' +
				'	<i class="fa-solid fa-file"></i>' +
				'	<div class="tooltip"><?= lang("Upload_Attachments"); ?></div>' +
				'</a>';

			var CVres = 'Not Uploaded';
			if( thsCV != '' ){
				CVres = '<a class="fileBtnView" target="_blank" href="../<?=$POINTER; ?>uploads/' + data[i]["faculty_cv"] + '" >View File</a>';
			}

			var CFres = 'Not Uploaded';
			if( thsCF != '' ){
				CFres = '<a class="fileBtnView" target="_blank" href="../<?=$POINTER; ?>uploads/' + data[i]["faculty_certificate"] + '" >View File</a>';
			}
			...
			var dt = '<div class="tr levelRow" id="faculty-' + data[i]["faculty_id"] + '">' +
				...
				'	<div class="td">' + CVres + '</div>' +
				'	<div class="td">' + CFres + '</div>' +
				'	<div class="td">' +
	...
	function editFacultyCV(qId) {

        var faculty_name = $('#faculty-' + qId + ' .faculty_name').val();
		

        $('#editFacultyCVForm .cv-faculty_id').val(qId);

        $('#editFacultyCVForm .cv-faculty_name').val(faculty_name);
		

		showModal('editFacultyCVModal');
	}
...
<!-- Modals START -->
<div class="modal" id="editFacultyCVModal">
	<div class="modalHeader">
		<h1><?= lang("Upload_Faculty_Attachments", "AAR"); ?></h1>
		<i onclick="closeModal();" class="fa-solid fa-times"></i>
	</div>
	<div class="modalBody">
		<div class="row pageForm" id="editFacultyCVForm">


        <input type="hidden" class="inputer cv-faculty_id" data-name="faculty_id" data-den="0" value="0" data-req="1"
						data-type="hidden">
			...
			<div class="col-1">
				<div class="formElement">
					...
					<input type="text" class=" cv-faculty_name" data-name="faculty_name" data-den="" data-req="1"
			...
			<div class="col-1">
				<div class="formElement">
					<label><?= lang("faculty_cv", "AAR"); ?></label>
					<input type="file" class="inputer cv-faculty_cv" data-name="faculty_cv" data-den="" data-req="1"
						data-type="file" accept="application/pdf">
				</div>
			</div>
			<!-- ELEMENT END -->
			...
			<div class="col-1">
				<div class="formElement">
					<label><?= lang("faculty_certificate", "AAR"); ?></label>
					<input type="file" class="inputer cv-faculty_certificate" data-name="faculty_certificate" data-den="" data-req="1"
						data-type="file" accept="application/pdf">
				</div>
			</div>
			<!-- ELEMENT END -->
			...
			<div class="col-2">
				<div class="formElement">
					<button class="submitBtn" type="button"
						onclick="submitForm('editFacultyCVForm', '<?= $editFacultyCVController; ?>');"><?= lang("Update_Faculty", "AAR"); ?></button>
				</div>
			</div>
			<!-- ELEMENT END -->
