...
$pageDataController = $POINTER . "get_rpl_list";
	...
	<script>
		async function bindData(data) {
			...
			for (i = 0; i < data.length; i++) {
				...
				if (thsStt == 1) {
					btns += '<a onclick="approveRequest(' + data[i]["request_id"] + ');" id="appBtn-' + data[i]["request_id"] + '" class="dataActionBtn hasTooltip">' +
						'	<i class="fa-solid fa-check"></i>' +
						'	<div class="tooltip"><?= lang("Approve"); ?></div>' +
						'</a>';
					btns += '<a onclick="rejectRequest(' + data[i]["request_id"] + ');" id="rejBtn-' + data[i]["request_id"] + '" class="dataActionBtn hasTooltip">' +
						'	<i class="fa-solid fa-times"></i>' +
						'	<div class="tooltip"><?= lang("Reject"); ?></div>' +
						'</a>';
				} else if (thsStt == 2 || thsStt == 5 || thsStt == 7) {
					btns += '<a href="<?= $POINTER; ?>rpl/013_form_view/?request_id=' + data[i]["request_id"] + '"class="dataActionBtn hasTooltip">' +
						'	<i class="fa-solid fa-eye"></i>' +
						'	<div class="tooltip"><?= lang("View_Form_Details"); ?></div>' +
						'</a>';
				}
