...
<script>
	async function bindData(data) {
		...
		for (i = 0; i < data.length; i++) {
			...
			if (stt == 1) {
				btns += '<a onclick="changeDAStt(' + data[i]["da_id"] + ');" class="dataActionBtn tableBtnWarning hasTooltip">' +
	...
	function changeDAStt(da_id) {
		//data[i]["da_id"]
		resetForm('changeStatusForm');
		$('.edit-da_id').val(da_id);
...
<div class="modal" id="changeStatusModal">
	...
	<div class="modalBody">
		<div class="row pageForm" id="changeStatusForm">
			...
			<input type="hidden" class="inputer edit-da_id" data-name="da_id" data-den="" data-req="1" value="0"
				data-type="hidden">
			...
			<div class="col-1">
				<div class="formElement">
					<label><?= lang("DA_Status", "AAR"); ?></label>
					<select class="inputer new-da_status_id" data-name="da_status_id" data-den="0" data-req="1"
						data-type="text">
						<option value="0" selected><?= lang(data: "Please_Select", trans: "AAR"); ?></option>
						<?php
						$qu_SEL_sel = "SELECT `da_status_id`, `da_status_name` FROM  `hr_disp_actions_status` ORDER BY `da_status_id` ASC";
						$qu_SEL_EXE = mysqli_query($KONN, $qu_SEL_sel);
						if (mysqli_num_rows($qu_SEL_EXE)) {
							while ($SEL_REC = mysqli_fetch_assoc($qu_SEL_EXE)) {
								$idd = (int) $SEL_REC['da_status_id'];
								$nnn = $SEL_REC['da_status_name'];
								?>
