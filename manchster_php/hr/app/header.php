...
$qu_employees_list_sel = "SELECT COUNT(`employee_id`) FROM  `employees_list` WHERE ((`is_new` = 1))";
...
if (mysqli_num_rows($qu_employees_list_EXE)) {
	...
	$totalNewEmployees = (int) $employees_list_DATA[0];
}
...
<aside id="mainAside">
	...
	<nav class="mainMenu">
		...
		<!-- Link Start -->
		<a href="<?= $POINTER; ?>employees/list/" class="navItem <?php if ($pageId == 700) {
			  echo 'navItemSelected';
		  } ?>">
			<i class="fa-solid fa-users"></i>
			<span class="navItemText"><?= lang("Employees", "AAR"); ?></span>
			<?php
			if ($totalNewEmployees > 0) {
				?>
				<div class="navBadge"><?= $totalNewEmployees; ?></div>
				<?php
			}
			?>
		...
		<!-- Link Start -->
		<a href="<?= $POINTER; ?>requests/list/" class="navItem <?php if ($pageId == 550) {
			  echo 'navItemSelected';
		  } ?>">
			<i class="fa-solid fa-folder-open"></i>
			<span class="navItemText"><?= lang("Requests", "AAR"); ?></span>
		</a>
		<!-- Link End -->
		<!-- ------------------------------------------------------ -->

		<!-- Link Start -->
		<a href="<?= $POINTER; ?>hr_documents/list/" class="navItem <?php if ($pageId == 800) {
			  echo 'navItemSelected';
		  } ?>">
			<i class="fa-solid fa-file"></i>
			<span class="navItemText"><?= lang("HR_Documents", "AAR"); ?></span>
		</a>
		<!-- Link End -->
		<!-- ------------------------------------------------------ -->
	...
	<nav class="subMenu">
		...
		<a id="appsBtn" onclick="toggleAppsNav();" class="navItem <?php if ($pageId == 15000) {
			echo 'navItemSelected';
		} ?>">
			<i class="fa-solid fa-cubes"></i>
			<span class="navItemText"><?= lang("Apps", "AAR"); ?></span>
		</a>
...
<div id="appsNav" class="appsNav appsNavHidden">
	...
	<a href="<?= $DIR_tasks; ?>" class="appNavItem">
		<i class="fa-solid fa-list"></i>
		<span class="navItemText"><?= lang("Tasks", "AAR"); ?></span>
	</a>

	<a href="<?= $DIR_calendar; ?>" class="appNavItem">
		<i class="fa-solid fa-calendar"></i>
		<span class="navItemText"><?= lang("Calendar", "AAR"); ?></span>
	</a>

	<a href="<?= $DIR_messages; ?>" class="appNavItem">
		<i class="fa-solid fa-envelope"></i>
		<span class="navItemText"><?= lang("Chats", "AAR"); ?></span>
	</a>

	<!-- Link Start -->
	<a href="<?= $DIR_tickets; ?>" class="appNavItem <?php if ($pageId == 550) {
		  echo 'navItemSelected';
	  } ?>">
